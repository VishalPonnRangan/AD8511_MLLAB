...
# load data
data = sio.loadmat('/Users/vishal/PycharmProjects/MLPCharacterRecognizer/matlab/emnist-byclass.mat')
...
y = tf.compat.v1.placeholder(tf.float32,shape = [None,62])

# Required Function definitions
# Weight and Bias initializers
def weight(name,shape):
    # using Xavier initialization
    initial = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
...
def conv2d(x,W,s):
    return tf.nn.conv2d(input=x,filters=W,strides = [1,s,s,1],padding = 'SAME')
...
h_d = tf.nn.dropout(h_b2,rate=1 - (keep_prob))
...
Decay_rate = 0.96
Decay_steps = 10
learning_rate = tf.compat.v1.train.exponential_decay(initial_lr, global_step, Decay_steps, Decay_rate, staircase=True)

# Evaluation metrics
cross_entropy = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(
    labels = tf.stop_gradient( y),logits = y_pred))
optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(cross_entropy)
prediction = tf.argmax(input=y_pred,axis=1)
correct_pred = tf.equal(prediction,tf.argmax(input=y,axis=1))
